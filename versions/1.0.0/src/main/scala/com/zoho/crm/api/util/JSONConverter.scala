...
class JSONConverter(commonAPIHandler: CommonAPIHandler) extends Converter(commonAPIHandler) {
  ...
  private def setDataValue(`type`: String, memberDetail: JSONObject, fieldValue: Any) : Any = {
      ...
      else if (memberDetail.has(Constants.STRUCTURE_NAME) && memberDetail.has(Constants.MODULE)) isRecordRequest(fieldValue, getModuleDetailFromUserSpecJSON(memberDetail.getString(Constants.MODULE)), null, memberDetail)
  ...
  override def getResponse(response: Any, packageName: String): Any = {
    ...
    if (responseString == null || responseString.equals("null") || responseString.isEmpty || responseString.trim().isEmpty || responseString.equals("{}")) return null
